...
public class Estimator {
	...
	//perplexity 追加  2015/05/24
	public double perplexity(){
		double wordcount = 0;
		double loglik = 0;
		
		for(int m = 0;m < trnModel.M;m++){ //文書分繰り返す
			for(int n =0; n < trnModel.data.docs[m].length ;n++){ //文書の単語数分繰り返す
				double sum = 0;
				for(int k =0; k < trnModel.K; k++){ //トピック数K
					sum += trnModel.theta[m][k] * trnModel.phi[k][trnModel.data.docs[m].words[n]];
				}
				loglik += Math.log(sum);
			}
			wordcount += trnModel.data.docs[m].length;
		}
	...
	public void estimate(){
		...
		computeTheta();
		computePhi();
		trnModel.perplexity = perplexity();
		trnModel.liter--;
